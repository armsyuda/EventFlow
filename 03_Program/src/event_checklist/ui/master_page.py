...
class MasterPage(QWidget):
    def __init__(self, db, parent=None, embedded: bool = False):
        ...
        configure_resizable_table(self.table, [60, 92, 116, 160, 240, 76, 76, 120, 90, 136, 126, 110, 72, 72, 90])
        self.table.cellChanged.connect(self._cell_changed)
        root.addWidget(self.table, 1)
        note = QLabel("셀을 한 번 누르면 바로 수정할 수 있습니다. 열 경계를 드래그하면 폭을 바꾸고, 열 제목을 끌면 순서도 바꿀 수 있습니다.")
    ...
    def refresh(self):
        ...
        for r, item in enumerate(rows):
            ...
            values = [item["major"], item["minor"], item["name"], item["detail"], quantity, item["unit"]]
            ...
            for c, value in [(12, f"{item['start_offset']:+d}"), (13, f"{item['due_offset']:+d}")]:
    ...
    def _update_field(self, item_id: int, field: str, value) -> None:
        if self.loading:
            return
        allowed = {"default_vendor_id", "default_assignee_id", "default_vat_type", "anchor", "priority"}
        if field not in allowed:
            return
        self.db.execute(f"UPDATE master_items SET {field}=? WHERE id=?", (value, item_id))

    def _cell_changed(self, row: int, column: int) -> None:
        if self.loading or column not in {1, 2, 3, 4, 5, 6, 7, 12, 13}:
            return
        cell = self.table.item(row, column)
        if cell is None:
            return
        item_id = cell.data(Qt.ItemDataRole.UserRole)
        fields = {1: "major", 2: "minor", 3: "name", 4: "detail", 5: "quantity", 6: "unit", 7: "base_unit_price", 12: "start_offset", 13: "due_offset"}
        field = fields[column]
        raw = cell.text().strip()
        try:
