...
def fetch_latest_release(timeout: float = 6.0) -> UpdateInfo:
    ...
    if not url.startswith(expected_prefix):
        raise UpdateCheckError(
            ...
            "확인 방법\nRelease에 이 프로젝트에서 직접 만든 EventFlow-Windows.zip을 다시 첨부하세요.\n\n"
...
def download_update(info: UpdateInfo, timeout: float = 90.0) -> Path:
    if not info.asset_url:
        raise UpdateDownloadError(
            "업데이트를 설치할 수 없습니다.\n\n확인된 원인\n공개 Release에 자동 설치용 EventFlow-Windows.zip이 없습니다.\n\n"
...
try {{
    ...
    Set-Content -LiteralPath '{ps(marker_path)}' -Encoding ASCII -Value 'EventFlow installed application'
