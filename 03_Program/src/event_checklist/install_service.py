...
def launch_fixed_installation(process_id: int) -> None:
    ...
    if not is_packaged_app():
        raise RuntimeError("고정 설치는 패키징된 EventFlow.exe에서만 실행할 수 있습니다.")
    ...
    if source_executable.name.casefold() != EXECUTABLE_NAME.casefold():
        raise RuntimeError("EventFlow.exe가 포함된 정상 배포 폴더에서 실행해 주세요.")
...
if (-not (Test-Path -LiteralPath (Join-Path $staging '{EXECUTABLE_NAME}'))) {{ throw '설치 파일에 EventFlow.exe가 없습니다.' }}
